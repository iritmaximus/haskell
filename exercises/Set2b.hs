...
countdown n
    | n > 0 = countdown' n "Ready!!"
    | otherwise = "Ready! Liftoff!"

countdown' :: Integer -> String -> String
countdown' n str
    | n > 0 = countdown' (n-1) (str ++ (show n) ++ "... ")
    | otherwise = str ++ "Liftoff!"
...
smallestDivisor n
    | n > 1 = smallestDivisor' n 2

smallestDivisor' :: Integer -> Integer -> Integer
smallestDivisor' n t
    | n > 1 = if n `mod` t == 0 then t
                else smallestDivisor' n (t+1)
    | otherwise = 0
...
isPrime n
    | n <= 1 = False
    | otherwise = if smallestDivisor n == n then True
                    else False
