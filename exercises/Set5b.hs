...
mapTree f (Node value t1 t2) = Node (f value) (mapTree f t1) (mapTree f t2)
...
cull val (Node value t1 t2)
  | val == value = Empty
  | otherwise = Node value (cull val t1) (cull val t2)
...
walk (step:steps) (Node a t1 t2) = case step of
  StepL -> walk steps t1
  StepR -> walk steps t2
